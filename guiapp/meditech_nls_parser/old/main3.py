...
def map():
    text = "C:/Users/germancz/Dropbox/test11.txt"
...
#    arr = (ptnamea,newlisto],[ptnameb,newlistt])
 #   for i in arr:
    ...
    for e in newlistt:
        testlen = re.findall(r"Specimen:\s+(\d{2}:\w+:\D+\w+)\s+SOUT\s+Received:\s+", e[6], re.S)
        if len(testlen) > 1:
            templist = []
            temp = re.findall(r"(Specimen:\s+\d{2}:\w+:\D+\w+\s+SOUT\s+Received:\s+"
                         "\d{2}\/\d{2}\/\d{2}-\d+\s+Spec\s{1}Type:\s+\w+\s+.+?TISSUES:\s+.+?CLINICAL HISTORY-.+?GROSS DESCRIPTION.+?"
                         "\*\*\*\s+FINAL DIAGNOSIS\s+\*\*\*.+?Final:\s+\D+\S\s+\d{2}\/\d{2}\/\d{2}\s+)", e[6], re.S)
            for o in temp:
                
                newvars = re.findall(r"Specimen:\s+(\d{2}:\w+:\D+\w+)\s+SOUT\s+Received:\s+"
                         "(\d{2}\/\d{2}\/\d{2})-\d+\s+Spec\s{1}Type:\s+\w+\s+.+?TISSUES:\s+(.+)CLINICAL HISTORY-(.+)GROSS DESCRIPTION(.+)"
                         "\*\*\*\s+FINAL DIAGNOSIS\s+\*\*\*(.+)Final:\s+(\D+\S)\s+(\d{2}\/\d{2}\/\d{2})\s+", o, re.S)
                for a in newvars:
                    li = list(a)
                    templist.append(li)
                    if e[0] in finaldict:
                        print "111newstuff: ", e[0]
                        finaldict[e[0]].append([e[0], e[1], e[2], e[3], e[4], e[5], templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5],
                               templist[0][6], templist[0][7]])
                        print "111newstuff: ", templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6], templist[0][7]
                    else:
                        print "1222newstuff: ", e[0]
                        finaldict[e[0]] = [e[0], e[1], e[2], e[3], e[4], e[5], templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6],
                                            templist[0][7]]
                        print "1222newstuff: ", templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6], templist[0][7]
        else:
            templist = []
            newvars = re.findall(r"Specimen:\s+(\d{2}:\w+:\D+\w+)\s+SOUT\s+Received:\s+"
                         "(\d{2}\/\d{2}\/\d{2})-\d+\s+Spec\s{1}Type:\s+\w+\s+.+?TISSUES:\s+(.+)CLINICAL HISTORY-(.+)GROSS DESCRIPTION(.+)"
            ...
            if e[0] in finaldict:
                ...
                finaldict[e[0]].append([e[0], e[1], e[2], e[3], e[4], e[5], templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5],
                       templist[0][6], templist[0][7]])
                print "2111newstuff: ", templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6], templist[0][7]
            else:
                print "2222newstuff: ", e[0], e[1]
                print e[6]
                # print templist
                finaldict[e[0]] = [e[0], e[1], e[2], e[3], e[4], e[5], templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6], templist[0][7]]
                print "2222newstuff: ", templist[0][0], templist[0][1], templist[0][2], templist[0][3], templist[0][4], templist[0][5], templist[0][6], templist[0][7]
    reduce (finaldict)

def reduce(finaldict):
        
    outxml = "C:/Users/germancz/Dropbox/outtest33.txt"
        ...
        for key in finaldict:
            if type(finaldict[key][0]) == str:
                ...
                xml = (tabinit
                            ...
                            + " "*4 + "<PATHOLOGIST>" + finaldict[key][12] + "</PATHOLOGIST>\n"
                            + " "*4 + "<SIGNED_OUT_DATE>" + finaldict[key][13] + "</SIGNED_OUT_DATE>\n" 
                            + tabend)
                print xml
                out.write(xml)
                ...
                for a in finaldict[key]:
                    ...
                    xml = (tabinit
                            ...
                            + " "*4 + "<GROSS_DESCRIPTION>" + "<![CDATA[\n" + a[10] + "\n]]>" + "</GROSS_DESCRIPTION>\n"
                            + " "*4 + "<FINAL_DIAGNOSIS>" + "<![CDATA[\n" + a[11] + "\n]]>" + "</FINAL_DIAGNOSIS>\n"
                            + " "*4 + "<PATHOLOGIST>" + a[12] + "</PATHOLOGIST>\n"
                            + " "*4 + "<SIGNED_OUT_DATE>" + a[13] + "</SIGNED_OUT_DATE>\n" 
                            + tabend)
                    print xml
                    out.write(xml)
...
def cleaner():
    ...
    filetext = "C:/Users/germancz/Dropbox/test.txt"
    outtext = "C:/Users/germancz/Dropbox/outtest.txt"
